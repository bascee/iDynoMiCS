...
public abstract class LocatedAgent extends ActiveAgent implements Cloneable 
{
	...
	@Override
	public void collision(LocatedAgent neighbour) {
		Volume.neighbourInteraction(neighbour.Points.get(0),
				Points.get(0), getRadius(true)+neighbour.getRadius(true));
	}
	...
	{
		if(Points.size() == 1)
			return Points.get(0).coord(getRadius(true));
	...
	{
		if(Points.size() == 1)
			return Points.get(0).dimensions(getRadius(true));
		float[] coord 		= coord();
		float[] upper 		= upper();
