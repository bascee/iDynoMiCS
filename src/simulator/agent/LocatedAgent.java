...
{
	/**
	 * (Mass)points are used in mechanical relaxation
	 */
    protected List<Point> _points;
    
    /**
     * The rest-lengths of the inner springs connected multiple points belonging
     * to the same agent
     */
    protected Double[] _lengths;
    
    /**
     * The rest angle between two line segments (torsion springs) a rest angle 
     * of 180° is assumed if no value is given
     */
	...
	public void mechMovement() {
		ContinuousVector diff = new ContinuousVector(_points.get(0).getPosition());
		diff.subtract(getLocation());
		this._movement.add(diff);
	}
	
	@Override
	public void refreshPoint(int nDim) 
	{
		_points = new LinkedList<Point>();
		Double[] loc = (nDim == 3 ? new Double[]{_location.x, _location.y,
				_location.z} : new Double[]{_location.x, _location.y } );
		_points.add(new Point(loc));
	}
	
	@Override
	public void updatePoint(int nDim) 
	{
		Double[] loc = (nDim == 3 ? new Double[]{_location.x, _location.y,
				_location.z} : new Double[]{_location.x, _location.y } );
		_points.get(0).setPosition(loc);
	}
	
	@Override
	public void collision(LocatedAgent neighbour, Double[] gridLengths) {
		Volume.neighbourInteraction(neighbour._points.get(0),
				_points.get(0), getRadius(true)+neighbour.getRadius(true), gridLengths);
	}
	
	@Override
	public Double euStep(Double vSquare, double dt) 
	{
		for(Point a: _points)
			vSquare = a.euStep(vSquare, dt, getRadius(true));
		return vSquare;
	}
	...
	{
		if(_points.size() == 1)
			return _points.get(0).coord(getRadius(true));
		float[] coord = new float[nDim()];
		for (Point o: _points) 
			for (int i = 0; i < nDim(); i++) 
				coord[i] = Math.min(coord[i], o.coord(getRadius(true))[i]);
		return coord;
	}
	
	@Override
	public float[] coord(double t) 
	{
		if(_points.size() == 1)
			return _points.get(0).coord(getRadius(true));
		float[] coord = new float[nDim()];
		for (Point o: _points) 
			for (int i = 0; i < nDim(); i++) 
				coord[i] = Math.min(coord[i], o.coord(getRadius(true))[i]) - (float) t;
		return coord;
	}
	...
	public float[] upper() 
	{
		...
		for (Point o: _points) 
			for (int i = 0; i < nDim(); i++) 
				upper[i] = Math.max(upper[i], o.upper(getRadius(true))[i]);
		return upper;
	}
	...
	{
		if(_points.size() == 1)
			return _points.get(0).dimensions(getRadius(true));
		float[] coord 		= coord();
		float[] upper 		= upper();
	...
	@Override
	public float[] dimensions(double t) 
	{
		if(_points.size() == 1)
			return _points.get(0).dimensions(getRadius(true));
		float[] coord 		= coord();
	...
	public int nDim() 
	{
		return _points.get(0).nDim();
	}
	
	@Override
	public void innerSprings() 
	{
		double kspring = 0.2;
		for (int i = 0; i<_points.size()-1; i++) 
		{
			Point a 		= _points.get(i);
			Point b 		= _points.get(i+1);
			Double[] diff 	= Vect.minus(a.getPosition(), b.getPosition());
			double dn 		= Vect.normE(diff);
			
			//FIXME: Bas - the calculation of the spring force (Fs) still 
			// requires some fine tuning/ calibration.
			Double[] Fs 	= Vect.product(diff, kspring*(Math.abs((dn - 
					_lengths[i])) * (dn - _lengths[i])));
		
			b.setForce(Vect.sum(b.getForce(),Fs));
			a.setForce(Vect.sum(a.getForce(),Vect.inverse(Fs)));
		}
	}
