...
public final class Volume {
	 ...
	 * 
	 */
	public static void cyclicDeltaP(Double[] gridLengths)
	{
		for (int i = 0; i < deltaP.length; i++)
		if ( Math.abs(deltaP[i]) > 0.5 * gridLengths[i] )
			deltaP[i] -= Math.signum(deltaP[i]) * gridLengths[i];
	}
	 ...
	 * 			sum of radii
	 */
	public static void neighbourInteraction(Point a, Point c, Double radii, Double[] gridLengths) 
	{
		Double[] force = interact(pointPoint(a.getPosition(), c.getPosition(), gridLengths), radii);
	...
	{
		double c;
		double p 			= 0.1; 			// pull distance (this may mess up with eps because of the scale)
		double fPull 		= 0.001;		// pull force scalar
		double fPush 		= 2.0;			// push force scalar
		boolean exponential = true; 		// exponential pull curve
		distance 			-= radii+0.1;	// added margin
		
		//repulsion
		if (distance < 0.0) 
		{
			c = fPush * distance * distance;
			return Vect.product(Vect.normalize(deltaP),c);
		} 
		...
		else if (distance < p) 
		{
			...
			return Vect.product(deltaP,c);
		}
		return Vect.zeros(deltaP.length);
	}
	 ...
	 */
	public static Double pointPoint(Double[] p, Double[] q, Double[] gridLengths) 
	{
		deltaP = Vect.minus(p,q);
		if (gridLengths != null)
			cyclicDeltaP(gridLengths);
		return Vect.normE(deltaP);
	}
	...
	public static Double linesegPoint(Double[] p0, Double[] p1, Double[] q0) 
	{
		...
		deltaP = Vect.minus(Vect.sum(p0, Vect.product(ab,s)), q0);
		return Vect.normE(deltaP);
	}
