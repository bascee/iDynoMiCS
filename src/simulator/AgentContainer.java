...
public class AgentContainer 
{
	...
	public void step(Simulator aSim)
	{
		...
		while (elapsedTime < globalTimeStep)
		{
			...
			*/
			...
			// up by code that did exactly the same thing, seems silly.
			nDead += removeAllDead();
		...
		LogFile.chronoMessageOut("Agents stepped/dead/born: " + nDead + "/"
	...
	private void mechanicalRelaxation() {
		...
		while(tMech < tStep) 
		{
			...
			{
				//agent.innerSprings();
				for(SpecialisedAgent neighbour: agentTree.search(agent.coord(0.1), 
						agent.dimensions(0.1))) 
				{
					if (agent.identifier() > neighbour.identifier())
						agent.collision((LocatedAgent) neighbour);
				}
			}
